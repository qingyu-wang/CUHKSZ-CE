...
def preprocess_wenjuanxing_1(old_wb_path, new_wb_path):
    result = {
        "msgs": [],
    }
    ...
    try:
        ...
        assert old_titles[2] == "邮箱"

        custom_titles = ["[自定义] %s" % i for i in old_titles[3:]]
        ...
        for idx, value in enumerate(old_values):
            ...
            mail_1 = str(value[2]).strip()
            # campus_idno_2 = mail_1.split("@link.cuhk.edu.cn")[0]
            campus_idno_2 = mail_1.split("@")[0]

            for i, custom_title in enumerate(custom_titles):
                info[custom_title] = value[i+3]

            # 核对校园卡号
            find_student_num_by_idno_1 = mongo.coll_user_info.count_documents({"campus_role": "学生", "campus_idno": campus_idno_1})
            find_student_num_by_idno_2 = mongo.coll_user_info.count_documents({"campus_role": "学生", "campus_idno": campus_idno_2})
            if find_student_num_by_idno_1 > 1:
                ...
                continue
            if find_student_num_by_idno_2 > 1:
                print("find student error [ find_student_num_by_idno_2=%s, campus_idno_2=%s ]" % (find_student_num_by_idno_2, campus_idno_2))
                continue
            if campus_idno_1 == campus_idno_2:
                ...
                if find_student_num_by_idno_1 != 0 and find_student_num_by_idno_2 == 0:
                    info["校园卡号"] = campus_idno_1
                elif find_student_num_by_idno_2 != 0 and find_student_num_by_idno_1 == 0:
                    info["校园卡号"] = campus_idno_2
                elif find_student_num_by_idno_1 != 0 and find_student_num_by_idno_2 != 0:
                    info["校园卡号"] = "卡号未知"
                    info["系统备注"] += "%s卡号不一致 (%s | %s)" % (
                        "" if len(info["系统备注"]) == 0 else "\n",
                        campus_idno_1,
                        campus_idno_2
                    )
            ...
            if info["校园卡号"] == "卡号未知":
                ...
                if info["姓名"] != "姓名未知":
                    ...
                    if campus_idno_1 in find_campus_idnos_by_name and campus_idno_2 not in find_campus_idnos_by_name:
                        info["校园卡号"] = campus_idno_1
                    elif campus_idno_2 in find_campus_idnos_by_name and campus_idno_1 not in find_campus_idnos_by_name:
                        info["校园卡号"] = campus_idno_2
                    else:
                        info["系统备注"] += "%s校园卡号与姓名不一致 (%s | %s | 系统: %s)" % (
                            "" if len(info["系统备注"]) == 0 else "\n",
                            campus_idno_1,
                            campus_idno_2,
    ...
    try:
        ...
        for i in range(3, num_cols):
            new_ws.column_dimensions[get_column_letter(i+2)].width = 20
...
if __name__ == "__main__":
    ...
    """
python -m utils.utils_tool_wenjuanxing1
    """

    old_wb_path = "./data/test/tool_utils-wenjuanxing-unprocessed.xlsx"
    new_wb_path = "./data/test/tool_utils-wenjuanxing-1.xlsx"

    result = preprocess_wenjuanxing_1(old_wb_path, new_wb_path)
